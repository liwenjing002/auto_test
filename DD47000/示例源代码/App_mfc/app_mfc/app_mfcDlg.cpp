...
void Capp_mfcDlg::OnBnClickedButtonfindfile()
{
	...
	if (  dlg.DoModal() == IDOK)
	{
		...
		if ( ret == 2 )
		{
			AfxMessageBox(L"增强功能可用");
		}

		if ( ret >= 1 )
		{
			ret = dd.DD_chk();  //检测驱动时是否加载成功
	    }
...
void Capp_mfcDlg::SetHotKey(void)
{
	...
	::RegisterHotKey(GetSafeHwnd(),6690,                0           ,  VK_F9 ); 
...
void Capp_mfcDlg::UnHotKey(void)
{
	...
	::UnregisterHotKey(GetSafeHwnd() ,6690);
}
...
//增强功能
void Capp_mfcDlg::Demo()
{
	//取当前激活窗口句柄,当用普通api无法获取时可用这个函数
	HWND h = dd.DD_GetActiveWindow();

	//以窗口坐标取色 , h==0时表示全屏坐标
	COLORREF clr = dd.DD_PickColor(h, 200,200 ,0);

	//抓图. 保存目录 c:\dd snap
	int ret = dd.DD_SnapPic(h,100,100, 100,200);

	//以窗口坐标鼠标移动. h==0时表示全屏坐标,等同 dd.mov
	ret = dd.DD_MouseMove(h,200,200);
}
